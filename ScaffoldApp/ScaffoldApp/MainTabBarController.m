...
@property (nonatomic, strong) LOTAnimationView *msgAddAnimation;
...
//    [self customizeInterface];
    [self setupMsgAddAnimationView];
    [self updateMessageBadge];
    
}

- (NSArray *)viewControllersForTabBar {
    KLHomeViewController *firstViewController = [[KLHomeViewController alloc] init];
    UIViewController *firstNavigationController = [[CYLBaseNavigationController alloc]
                                                   initWithRootViewController:firstViewController];
    [firstViewController cyl_setHideNavigationBarSeparator:YES];
    // [firstViewController cyl_setNavigationBarHidden:YES];
    KLShopViewController *secondViewController = [[KLShopViewController alloc] init];
    ...
    KLMessageViewController *thirdViewController = [[KLMessageViewController alloc] init];
    ...
    KLMineViewController *fourthViewController = [[KLMineViewController alloc] init];
...
- (void)updateMessageBadge {
    ...
    [self setViewDidLayoutSubViewsBlockInvokeOnce:YES block:^(CYLTabBarController *tabBarController) {
        ...
        dispatch_after(when, dispatch_get_main_queue(), ^{
            @try {
                NSString *testBadgeString = @"80";
                UIViewController *msgvc = tabBarController.viewControllers[2];
                [msgvc cyl_showBadgeValue:testBadgeString animationType:CYLBadgeAnimationTypeNone];
                
                weakSelf.msgAddAnimation = ({
                    LOTAnimationView *lotview = [LOTAnimationView animationNamed:@"tab_new_msg" inBundle:[NSBundle mainBundle]];
                    lotview.loopAnimation = YES;
                    lotview.contentMode = UIViewContentModeScaleAspectFill;
                    lotview.animationSpeed = 1.0;
                    lotview.backgroundColor = [UIColor systemRedColor];
                    lotview;
                });
                
                [msgvc.cyl_tabButton addSubview:weakSelf.msgAddAnimation];
                weakSelf.msgAddAnimation.frame = CGRectMake(30, 0, 100, 50);
                
//                [weakSelf.msgAddAnimation mas_makeConstraints:^(MASConstraintMaker *make) {
//                    make.centerX.mas_equalTo(msgvc.cyl_tabButton.cyl_badge.mas_centerX);
//                    make.centerY.mas_equalTo(msgvc.cyl_tabButton.cyl_badge.mas_centerY);
//                    make.width.height.mas_equalTo(30);
//                }];
                [weakSelf.msgAddAnimation play];
...
- (void)setupMsgAddAnimationView {
    __weak typeof(self)weakSelf = self;
    [self setViewDidLayoutSubViewsBlockInvokeOnce:YES block:^(CYLTabBarController *tabBarController) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            /*
            weakSelf.msgAddAnimation = ({
                LOTAnimationView *lotview = [LOTAnimationView animationNamed:@"tab_new_msg"];
                lotview.loopAnimation = NO;
                lotview.contentMode = UIViewContentModeScaleAspectFill;
                lotview.animationSpeed = 1.0;
                lotview.backgroundColor = [UIColor blueColor];
                lotview;
            });
            ...
            [weakSelf.msgAddAnimation mas_makeConstraints:^(MASConstraintMaker *make) {
                make.center.mas_offset(msgvc.cyl_tabButton.cyl_badge);
            ...
            [msgvc.cyl_tabButton addSubview:view];
